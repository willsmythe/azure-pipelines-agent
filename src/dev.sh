...
{
    heading "Building ..."
    dotnet msbuild -t:Build -p:RuntimeIdentifier="${RUNTIME_ID}" -p:PackageRuntime="${RUNTIME_ID}" -p:BUILDCONFIG="${BUILD_CONFIG}" -p:AgentVersion="${AGENT_VERSION}" || failed build
...
function layout ()
{
    ...
    dotnet msbuild -t:layout -p:RuntimeIdentifier="${RUNTIME_ID}" -p:PackageRuntime="${RUNTIME_ID}" -p:BUILDCONFIG="${BUILD_CONFIG}" -p:AgentVersion="${AGENT_VERSION}" || failed build
...
function runtest ()
{
    ...
    dotnet msbuild -t:test -p:RuntimeIdentifier="${RUNTIME_ID}" -p:PackageRuntime="${RUNTIME_ID}" -p:BUILDCONFIG="${BUILD_CONFIG}" -p:AgentVersion="${AGENT_VERSION}" || failed "failed tests" 
}
